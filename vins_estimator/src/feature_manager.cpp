...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double td)
{
    ...
    double parallax_sum = 0;//parallax视差
    int parallax_num = 0;
    last_track_num = 0;
    // 遍历每个特征点
    ...
    {
        // 用特征点信息构造一个对象
        FeaturePerFrame f_per_fra(id_pts.second[0].second, td);//td：传感器延时
    ...
    for (auto &it_per_id : feature)
    {
        ...
        if (it_per_id.start_frame <= frame_count - 2 &&
            it_per_id.start_frame + int(it_per_id.feature_per_frame.size()) - 1 >= frame_count - 1)//中间那个int里面表示持续帧数
        {
            parallax_sum += compensatedParallax2(it_per_id, frame_count); //计算归一化相机坐标系下的视差，即匹配点之间的距离，最终得到两幅图像之间特征点移动的总距离
            parallax_num++;//有效追踪点的对数
        }
    }
    // 这个和上一帧没有相同的特征点
    if (parallax_num == 0)
