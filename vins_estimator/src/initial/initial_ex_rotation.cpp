...
{
    frame_count++; //这里的frame count指参加初始化过程的所有帧，并不仅限于一个窗口的范围
    // 根据特征关联求解两个连续帧相机的旋转R12
    Rc.push_back(solveRelativeR(corres));
    Rimu.push_back(delta_q_imu.toRotationMatrix());
    // 通过外参把imu的旋转转移到相机坐标系 相当于一个重投影的过程
    ...
    Matrix<double, 4, 1> x = svd.matrixV().col(3); //AX = 0 svd分解求得的X的值，对应V矩阵的第三列
    ...
    {
        calib_ric_result = ric; // 标定成功的旋转外参
        return true;
    }
    else
        return false;
