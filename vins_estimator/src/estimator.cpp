...
void Estimator::setParameter()
{
    ...
    // 这里可以看到虚拟相机的用法
    ProjectionFactor::sqrt_info = FOCAL_LENGTH / 1.5 * Matrix2d::Identity(); //信息矩阵，表征置信度为1.2个像素
...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    // 滑窗中保留11帧，frame_count表示现在处理第几个图像帧，一般处理到第11帧时就保持不变了
    // 由于预积分是帧间约束，因此第1个预积分量实际上是用不到的
    if (!pre_integrations[frame_count]) //如果没有数据就new一个
    {
    ...
    if (frame_count != 0)
    {
        pre_integrations[frame_count]->push_back(dt, linear_acceleration, angular_velocity);//这边push back之后，所有的预积分相关量已经存在成员变量里面了
        //if(solver_flag != NON_LINEAR)
            // 这个量用来做初始化用的
            tmp_pre_integration->push_back(dt, linear_acceleration, angular_velocity);
        // 保存传感器数据
        dt_buf[frame_count].push_back(dt);//i到i+1的时间
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const std_msgs::Header &header)
{
    ...
    if (f_manager.addFeatureCheckParallax(frame_count, image, td)) //计算归一化相机坐标系下的视差，true则表明是前两帧或者和上一帧追踪点数小于20或上一帧视差大，被选为关键帧
    ...
    if(ESTIMATE_EXTRINSIC == 2) 
    {
        ...
        {
            // 这里标定imu和相机的旋转外参的初值
            // 因为预积分是相邻帧的约束，因为这里得到的图像关联也是相邻的，得到相邻图像特征点的约束
...
bool Estimator::initialStructure()
{
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            // 索引以及各自坐标系下的坐标
            tmp_feature.observation.push_back(make_pair(imu_j, Eigen::Vector2d{pts_j.x(), pts_j.y()})); //特征点所在帧，特征点坐标，得到同一个特征点在所有帧下的坐标
        }
        sfm_f.push_back(tmp_feature);
    } 
    Matrix3d relative_R;
    Vector3d relative_T; //枢纽帧和最后一帧之间的坐标变换
    int l; //枢纽帧在滑窗中的索引
    if (!relativePose(relative_R, relative_T, l)) //找枢纽帧，计算枢纽帧到最后一帧的位姿变换，枢纽帧为单位阵
    {
...
void Estimator::optimization()
{
    ...
    // 把优化后double -> eigen 为什么只重新确定yaw？因为IMU的加入，roll和pitch都是可观的，只有yaw会出现零空间漂移的情况
    double2vector();
    // Step 4 边缘化
    // 科普一下舒尔补
