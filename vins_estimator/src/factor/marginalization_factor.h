...
{
    // 残差块管理：构造函数需要cost function（约束），loss function：残差的计算方式，相关联的参数块，待边缘化的参数块的索引
    ...
    std::vector<double *> parameter_blocks; // 边缘化有关的参数块
    std::vector<int> drop_set; // 需要被边缘化的参数块在parameter_blocks中的索引
