...
bool ProjectionFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    if (jacobians)
    {
        ...
        if (jacobians[0])
        {
            ...
            jacobian_pose_i.rightCols<1>().setZero(); //这是因为代码里面四元数的顺序是xyzw，所以最后一列为0表示实部对旋转求导为0
        }
...
#if 1
            jacobian_feature = reduce * ric.transpose() * Rj.transpose() * Ri * ric * pts_i * -1.0 / (inv_dep_i * inv_dep_i);// pts_i为归一化相机坐标系下的坐标
#else
