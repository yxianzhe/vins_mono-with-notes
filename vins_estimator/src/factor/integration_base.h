...
{
  public:
    IntegrationBase() = delete; //禁止默认构造函数的生成
    IntegrationBase(const Eigen::Vector3d &_acc_0, const Eigen::Vector3d &_gyr_0,
                    const Eigen::Vector3d &_linearized_ba, const Eigen::Vector3d &_linearized_bg) //参数：加速度值，陀螺仪值，bias_a bias_w; 构造函数进行初始化
    ...
    {
        noise = Eigen::Matrix<double, 18, 18>::Zero(); // 噪声的协方差矩阵
        noise.block<3, 3>(0, 0) =  (ACC_N * ACC_N) * Eigen::Matrix3d::Identity(); // nak，这是读进来的数据，加速度计噪声的方差
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        // 首先中值积分更新状态量，这里对应的是离散化预积分量的表示那三个公式
        ...
        if(update_jacobian)
        {
            ...
            //step_jacobian = F;
            //step_V = V;
            jacobian = F * jacobian;//F矩阵更新，因为我们最终需要的是两个图像帧之间的雅克比矩阵
            covariance = F * covariance * F.transpose() + V * noise * V.transpose();//协方差矩阵更新
        }
    ...
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        //由于误差的引入，使得计算的变换四元数的模不再等于1，变换四元数失去规范性
        delta_q.normalize();
        sum_dt += dt;
        acc_0 = acc_1;
        gyr_0 = gyr_1;  
    ...
    // 计算和给定相邻帧状态量的残差，这里是指关键帧之间的了，返回值就是最小二乘里面的残差，用于优化
    ...
    double dt;//i到i+1时刻的时间
    Eigen::Vector3d acc_0, gyr_0; //i时刻的加速度和陀螺仪
    Eigen::Vector3d acc_1, gyr_1; //i+1时刻的加速度和陀螺仪
