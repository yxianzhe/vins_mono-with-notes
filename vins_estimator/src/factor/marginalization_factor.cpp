...
void MarginalizationInfo::addResidualBlockInfo(ResidualBlockInfo *residual_block_info)
{
    ...
    {
        double *addr = parameter_blocks[i]; //该参数块的首地址
        int size = parameter_block_sizes[i]; //该参数块的大小
        // 这里是个unordered map，避免重复添加
        parameter_block_size[reinterpret_cast<long>(addr)] = size;  // 地址->global size reinterpret_cast将值映射到索引，哈希表中常用
    }
    // 待边缘化的参数块
