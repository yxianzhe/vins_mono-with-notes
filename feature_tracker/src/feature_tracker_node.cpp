...
void img_callback(const sensor_msgs::ImageConstPtr &img_msg)
{
    ...
    if (round(1.0 * pub_count / (img_msg->header.stamp.toSec() - first_image_time)) <= FREQ)    // 保证发给后端的不超过这个频率
    {
        ...
        // 这段时间的频率和预设频率十分接近，就认为这段时间很棒，重启一下，避免delta t太大 这个条件保证短时间内的发送频率不会大爆发
    ...
    cv::Mat show_img = ptr->image; //到这里完成了把ros图像话题转化为opencv中cv::Mat的操作，接下来就是图像处理的部分了
    TicToc t_r; //计时器写了一个类，构造函数里计时开始
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ROS_DEBUG("processing camera %d", i);
        if (i != 1 || !STEREO_TRACK) //STEREO_TRACK = false 是双目但仅对左目，或者不是双目的话
            trackerData[i].readImage(ptr->image.rowRange(ROW * i, ROW * (i + 1)), img_msg->header.stamp.toSec());
        else //如果是双目，对于右目相机，当然这里是没有用到的
        {
            if (EQUALIZE)
            {
        ...
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            ...
            for (unsigned int j = 0; j < ids.size(); j++)
            {
                ...
                if (trackerData[i].track_cnt[j] > 1)
                {
                    ...
                    id_of_point.values.push_back(p_id * NUM_OF_CAM + i); // 这样每个相机拥有的特征点id都不一样，当然对于单目没有这个问题
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle n("~"); // 声明一个句柄，～代表这个节点的命名空间，即feature_tracker这个节点作为命名空间
    ros::console::set_logger_level(ROSCONSOLE_DEFAULT_NAME, ros::console::levels::Info);    // 设置ros log级别
    readParameters(n); // 读取配置文件，传这个命名空间参数的目的是为了在launch文件中找到这个节点下的config_file参数，从而读取
