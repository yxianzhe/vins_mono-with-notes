...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
	...
	if (loop_index != -1)   // 代表找到了有效的回环帧
	{
        ...
        if (cur_kf->findConnection(old_kf)) // 如果确定两者回环 用的是描述子匹配+ransacPNP
        {
            // 更新最早回环帧，用来确定全局优化的范围
            ...
            // T_w_old * T_old_cur = T_w_cur,这就是回环矫正后当前帧的位姿 因为是要在固定回环帧的基础上进行pose graph矫正所有回环帧之后的帧
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                for (int j = 1; j < 5; j++)
                {
                  if (i - j >= 0 && sequence_array[i] == sequence_array[i-j])
                  {
                    ...
                    relative_t = q_array[i-j].inverse() * relative_t; //基于VIO计算出来的
