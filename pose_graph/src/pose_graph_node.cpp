...
// 利用VIO进行重定位的结果进行修正,相当于通过在estimater的滑窗优化问题中加入回环的约束，从而得到一个全局一致性更好的T_loop_cur
...
void process()
{
    ...
    while (true)
    {
        ...
        if(!image_buf.empty() && !point_buf.empty() && !pose_buf.empty())
        {
            ...
            // 下面根据pose时间找时间戳同步的原图和地图点 最后一张图片的时间大于等于新pose时间，表示有pose来了
        ...
        if (pose_msg != NULL)   // 判断一下是否有效
        {
            ...
            if((T - last_t).norm() > SKIP_DIS)  // 要求KF相隔必要的平移距离
            {
                ...
                // 创建回环检测节点的KF 时间戳 第几次来创建回环的index，位姿，原图，地图点（3d，2d，像素），地图点索引，第几次sequence
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_imu_forward = n.subscribe("/vins_estimator/imu_propagate", 2000, imu_forward_callback); // imu积分数据
    ros::Subscriber sub_vio = n.subscribe("/vins_estimator/odometry", 2000, vio_callback); //VIO里程计输出
